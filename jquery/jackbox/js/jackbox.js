...
;(function($) {
	...
	loaded = (function() {
		...
		return function(event) {
			...
			if(isImage) {
				...
				oWidth = content.width() || content[0].width;
				oHeight = content.height() || content[0].height;
				setSize();
	...
	showContent = (function() {
		...
		return function() {
			...
			if(isImage && !isIE8) {
				Jacked.fadeIn(content[0], {duration: 1000});
	...
	resized = (function() {
		...
		return function(event) {
			...
			if(throttling) {
				...
				resizeThrottle = setTimeout(throttleResize, 100);
				return;
	...
	function loadContent() {
		...
		if(touch) swipeable = theType === "image" || theType === "html";
	...
	function closing() {
		...
		if(!touch) setTimeout(scrollback, 10);
	...
	function blurThumbLoaded() {
		...
		*/
		...
		StackBlurImage(imgId, canvasId, 19);
